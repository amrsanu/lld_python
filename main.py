from movie_booking.booking_system import BookingSystem
...
def main():
    ...
    # Add some movies
    movie1 = system.add_movie("The Matrix", 136, "Sci-Fi", "English", "1999-03-31", "A computer hacker learns from mysterious rebels about the true nature of his reality and his role in the war against its controllers.")
    movie2 = system.add_movie("Inception", 148, "Sci-Fi", "English", "2010-07-16", "A thief who steals corporate secrets through the use of dream-sharing technology is given the inverse task of planting an idea into the mind of a C.E.O.")

    # Add a cinema and screens
    cinema1 = system.add_cinema("Cineplex", "123 Movie Lane")
    screen1 = system.add_screen_to_cinema(cinema1.cinema_id, 1, 100) # Screen 1 with 100 seats
    screen2 = system.add_screen_to_cinema(cinema1.cinema_id, 2, 150) # Screen 2 with 150 seats
    ...
    show1 = system.add_show(movie1.movie_id, screen1.screen_id, show1_time.isoformat(), (show1_time + timedelta(minutes=movie1.duration)).isoformat(), 10.00)
    show2 = system.add_show(movie2.movie_id, screen2.screen_id, show2_time.isoformat(), (show2_time + timedelta(minutes=movie2.duration)).isoformat(), 12.00)
    ...
    if len(available_seats_show1) >= 2:
        seats_to_book = [available_seats_show1[0].seat_id, available_seats_show1[1].seat_id]
        ...
        if booking1:
            ...
            for booking_details in user_bookings:
                print(f"- Booking ID: {booking_details['booking_id']}, Status: {booking_details['status']}")
            ...
            available_seats_after_cancellation = system.find_available_seats(show1.show_id)
